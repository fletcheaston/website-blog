import{a as o,j as e,aU as k}from"./main-DdHQErvU.js";import{z as c}from"./index-mSkvzYyn.js";import{I as y}from"./view-input-7msPCYiV.js";import{s as b}from"./utils-BkWvjRoc.js";import{B as w}from"./button-CFX2dUjs.js";import{P as S,T as u}from"./code-block-aPXLCB2L.js";import{B as T}from"./layout-DvqNAMRu.js";import{H as h}from"./header-D86GcD9X.js";import{T as D}from"./table-Dgl1a-zm.js";import{S as g}from"./separator-LnusGYe2.js";const x=`67 69 71 72 75 78 76
4 6 7 9 11 12 12
20 22 25 27 28 29 33
35 37 39 42 43 46 48 53
46 49 48 51 53
31 33 31 34 31
83 86 85 88 89 91 91
74 75 78 81 80 83 86 90
72 73 72 74 75 80
19 21 22 25 25 28
4 6 6 7 10 9
49 51 52 55 55 58 58
40 41 43 44 44 48
53 56 56 57 62
26 29 33 34 36 38
35 38 41 45 44
28 31 33 35 39 39
75 77 81 82 86
85 86 88 89 93 98
78 80 85 88 90
36 39 42 43 44 47 53 50
74 77 80 85 86 87 90 90
51 53 54 57 59 64 68
15 17 23 25 32
46 45 47 49 51
25 23 26 27 30 27
51 50 51 52 53 53
51 50 52 55 56 59 63
40 38 40 42 43 48
23 21 22 19 22 25
22 20 22 25 24 27 30 28
95 92 94 95 93 93
78 75 77 76 80
31 30 31 33 31 32 37
12 10 12 15 15 18
67 66 68 68 67
67 64 64 66 66
71 70 72 72 74 75 78 82
15 14 14 16 19 20 25
50 47 48 50 52 56 58 59
5 4 5 8 12 15 14
37 34 36 40 42 45 45
5 2 4 5 9 10 11 15
11 8 12 13 15 20
25 22 25 32 34 37 38 41
6 4 7 10 16 13
95 92 97 99 99
17 16 23 24 28
78 75 80 81 86
11 11 14 16 17
40 40 43 45 48 50 47
37 37 40 41 42 43 46 46
90 90 92 94 98
35 35 36 38 43
74 74 77 76 79 82 84 87
23 23 25 27 26 23
94 94 95 98 95 96 97 97
70 70 68 70 74
2 2 1 2 8
46 46 48 48 51
69 69 72 74 77 79 79 77
49 49 52 54 57 57 57
28 28 29 29 33
51 51 53 56 56 59 61 67
27 27 29 33 36 39 42
29 29 30 34 37 38 41 39
34 34 35 36 40 42 42
22 22 26 28 32
7 7 10 14 20
35 35 38 39 46 48
23 23 24 26 29 36 35
27 27 29 35 35
61 61 66 69 70 74
26 26 32 34 36 43
41 45 46 48 49 51
5 9 11 13 14 16 17 15
72 76 79 81 83 86 87 87
64 68 69 72 75 76 79 83
27 31 33 36 37 40 46
85 89 92 93 94 91 92
65 69 66 67 68 70 72 71
84 88 91 94 93 93
78 82 79 82 83 84 86 90
15 19 20 19 24
41 45 48 50 53 53 56 59
27 31 32 32 31
84 88 88 90 90
8 12 12 13 17
84 88 89 89 95
58 62 64 68 69 71
1 5 9 12 15 14
57 61 62 65 68 69 73 73
1 5 9 10 14
46 50 53 57 63
63 67 68 69 74 75 77
55 59 66 68 70 68
6 10 11 17 17
76 80 82 88 89 90 94
73 77 82 85 90
12 18 19 22 24
47 54 55 58 60 62 59
4 9 11 14 14
64 70 71 72 73 77
80 86 88 90 95
61 68 71 68 71 74 76
79 85 87 84 85 88 86
51 56 57 55 58 58
37 42 41 44 45 48 50 54
74 79 76 78 80 82 88
61 66 66 67 70
37 43 46 49 49 48
77 82 85 86 88 91 91 91
26 33 33 35 36 37 39 43
42 49 49 52 58
39 44 47 50 52 56 58
36 41 43 44 47 48 52 51
42 49 50 54 54
63 70 73 75 77 81 85
74 80 84 87 93
75 80 83 86 93 94
4 10 13 14 19 21 20
65 72 75 80 83 86 89 89
64 70 73 74 80 81 82 86
20 25 30 33 38
57 56 54 51 48 49
88 87 86 83 83
55 52 49 48 45 42 40 36
59 56 54 53 50 49 44
28 26 25 22 21 20 21 19
10 9 7 5 7 10
44 42 40 43 43
50 47 50 48 47 44 43 39
79 76 78 77 74 69
32 29 29 27 25
30 29 26 23 20 20 22
61 58 58 56 55 54 51 51
80 77 76 76 72
56 54 51 51 44
64 61 58 56 55 51 50 47
28 26 25 21 20 18 16 19
94 93 91 89 86 82 82
76 74 73 70 69 65 64 60
76 75 71 70 63
66 63 62 57 56
55 53 50 44 47
53 52 49 46 44 42 37 37
66 64 63 57 53
41 39 37 36 33 28 26 21
21 24 21 19 17 14
67 70 68 65 64 62 61 62
52 53 52 49 49
85 86 85 84 83 81 77
30 32 29 27 25 20
77 80 81 80 77 76
96 99 98 95 92 94 96
87 88 91 90 90
63 65 64 63 66 65 61
76 78 77 75 76 70
50 51 48 48 47 46 45 44
73 76 75 75 78
56 57 56 56 55 54 52 52
53 56 54 54 51 49 47 43
64 67 67 66 60
56 58 55 53 52 48 45
44 45 44 41 37 40
15 17 16 12 12
58 60 58 57 54 50 47 43
52 55 51 48 41
83 86 81 78 77 75
46 47 45 44 43 37 40
59 60 53 51 50 48 48
23 26 25 24 23 17 13
54 55 48 46 45 43 41 34
14 14 11 10 7 5
80 80 78 75 74 72 69 71
63 63 62 60 58 58
65 65 64 63 59
98 98 95 94 92 89 87 81
97 97 98 97 96
95 95 97 96 97
45 45 44 42 40 38 41 41
6 6 7 5 1
22 22 23 22 17
47 47 46 44 44 43
23 23 21 21 20 19 21
30 30 27 27 24 24
82 82 82 81 77
51 51 49 49 48 46 41
16 16 13 9 7 5 4 1
82 82 78 77 75 73 76
89 89 87 83 80 80
64 64 61 57 55 51
59 59 55 54 52 50 44
36 36 30 28 25
33 33 30 29 23 20 23
13 13 7 6 6
51 51 48 47 41 39 35
92 92 86 84 82 81 78 72
97 93 92 89 88 85 83
58 54 52 50 49 47 49
42 38 36 34 31 31
86 82 80 79 76 75 71
92 88 87 86 84 77
55 51 48 47 45 48 46 45
40 36 37 35 36
20 16 15 12 15 12 12
51 47 48 45 42 38
83 79 77 78 76 75 69
27 23 21 18 15 15 13 11
80 76 74 71 70 70 73
78 74 72 72 71 69 68 68
29 25 24 24 21 17
67 63 61 61 55
98 94 90 87 85 83 82
98 94 90 87 86 87
44 40 36 33 33
96 92 90 86 82
49 45 42 38 31
70 66 65 58 56 53
49 45 42 41 39 32 33
37 33 32 27 26 25 25
34 30 29 27 25 24 18 14
26 22 19 18 13 12 5
59 53 51 49 46 43
28 21 18 16 14 11 9 12
21 16 15 12 11 11
28 22 21 20 17 14 10
49 43 40 37 31
14 7 6 8 5 2 1
42 36 34 32 35 37
49 42 41 43 40 39 38 38
61 54 57 54 53 50 47 43
97 90 92 91 89 86 85 79
36 29 27 25 25 24 21
28 23 23 20 18 16 17
33 27 26 26 23 20 20
90 83 83 80 76
34 28 28 25 22 17
61 54 50 47 46 44 43 42
22 16 13 10 7 3 5
73 68 64 63 63
44 37 36 32 30 29 25
85 79 78 75 71 65
64 57 52 51 50
54 47 42 39 40
88 82 80 77 71 71
61 55 54 49 45
94 89 88 82 76
43 40 37 34 31 28 27 30
21 25 28 32 34 35 33
54 57 59 58 57 55
27 31 33 35 39
10 14 16 16 17 18 22
66 69 70 69 72 74
21 27 29 31 34 41 45
44 45 51 53 56 59 60 64
73 74 71 65 62 61 62
54 51 49 46 42 39 34
81 83 82 75 72 70 66
58 57 60 63 63 65 63
72 71 68 64 64
89 83 77 74 74
42 45 48 52 51
16 16 18 18 22
62 69 72 75 76 76 77 80
10 10 13 13 18
53 59 62 64 64 65 67 71
73 70 68 66 67 65 61
95 92 89 87 85 80 79 78
2 2 3 9 9
77 76 71 70 66
46 42 41 41 39 36 32
57 58 57 53 50 50
82 78 77 75 73 67 68
71 69 73 76 77 75
58 64 65 67 66 68 71
95 91 88 86 82 83
38 38 35 32 32
39 42 45 48 50 54 56 61
82 88 91 92 90
51 47 45 42 41 39 37 38
86 88 88 86 84 83 83
62 62 58 56 54 54
65 58 55 54 53 49
90 86 85 82 79
34 37 35 33 32 31 24 24
93 89 86 89 91
29 36 37 44 47 44
32 35 33 32 29 26 25 22
47 43 42 38 38
80 82 83 82 82
8 8 9 10 11 14 16 18
16 14 17 19 20 21 28 25
27 30 29 26 22
65 64 61 60 57 53 51
13 20 22 26 26
47 42 41 39 37 35 33 33
45 47 53 54 55 60
66 62 60 58 55 56 54 50
63 67 69 73 75 78 79 79
4 6 7 10 12 9 10 14
32 27 24 20 17 15 10
39 39 38 36 35 37 33
63 65 67 64 67
24 24 22 23 22
13 20 21 22 26
27 27 24 22 21 21 18 20
26 29 31 32 34 33
76 76 75 74 71 68 64 62
10 10 12 10 9 10
47 40 37 36 34 37
48 48 46 45 42 42 37
58 58 57 51 48 42
23 19 17 14 7 4
64 57 55 50 47 46 49
59 62 60 57 55 52 50 52
55 50 50 47 45 39
69 65 63 65 64 63 62
55 56 54 50 48 47 50
20 20 15 12 10 9
38 37 37 40 41 44 46 46
65 68 70 67 68 68
5 7 13 16 19
72 71 76 79 85
86 81 78 75 73 76 73 67
80 74 73 70 72 73
71 67 60 59 58 55 52 52
56 60 62 61 64 66 67 71
79 74 71 67 65 64
41 37 33 32 25
96 90 88 86 80 78 76 72
47 44 46 49 48 50 53
12 19 23 24 27 28
95 97 95 92 89 83
47 44 46 49 50 57 58 58
63 67 70 70 68
34 30 27 26 19
19 23 28 31 29
46 53 54 58 61 66
14 17 14 11 11 7
77 77 75 74 73 71 68
61 61 63 67 72
59 55 53 50 47 45 45
93 91 92 89 88
55 48 46 47 45 43 40 40
52 46 45 42 38 34
76 73 72 69 65 63 64
45 45 43 41 42
69 69 66 63 59 56 52
34 40 41 44 47 50 50
42 42 45 52 55 57 64
64 68 70 73 79
66 60 59 56 53 53 50 46
51 50 52 54 56 57 61
52 59 62 62 63 66 64
84 84 85 88 91 94 97 97
57 57 60 59 58 58
25 22 19 20 21 22 26
67 65 63 61 58 57 56 50
61 61 62 66 68 71
86 79 77 75 76 73 69
2 3 6 6 8 10 7
17 22 24 27 32
17 17 10 9 6 5 7
73 69 72 71 65
21 17 15 13 15 12 11 11
28 31 34 38 42
57 57 56 53 50 48 48 44
71 69 70 71 72 73 74 81
2 6 6 7 9
7 11 12 15 16 16 18 18
24 20 19 18 16 11 9 5
85 81 79 77 75 73 69
23 24 27 29 30 32 32 32
36 29 26 26 24 21
6 5 8 11 12 12 15 17
14 9 7 6 6 6
28 25 22 22 21
74 75 73 71 73 71 67
46 50 51 56 58 61 62 64
16 23 25 29 32 34 35 39
37 37 43 45 42
26 26 32 34 38
11 11 13 14 15 18 18 19
75 77 74 70 67 63
68 74 71 73 76 79 82 87
69 69 71 70 67 61
77 80 81 81 85
36 39 36 33 33 32 29 30
18 20 23 23 25 26
80 86 88 85 89
22 21 18 16 13 13
64 64 63 61 56
25 23 24 24 27 33
16 16 17 19 18 19
62 58 58 57 55 54
20 25 27 32 32
38 33 32 31 28 26 20
77 76 75 73 70 63 63
44 49 50 56 57 58 60
48 53 56 58 60 60 63 63
65 60 59 58 58 57 59
35 36 38 39 42 49
17 13 11 11 8 8
67 71 75 78 84
81 81 82 85 87 90 91 95
69 69 72 74 77 80 86
59 58 57 57 59
57 60 61 64 67 66 71
59 58 61 64 65 67 64
51 51 53 56 53 53
24 26 27 31 33 34
26 27 30 32 35 35
82 85 83 82 79 76 76 69
60 56 54 52 51 45 42 35
20 13 14 11 10
73 71 68 67 64 61 57
28 29 28 24 18
54 47 45 39 38 35
41 43 46 50 53 53
32 36 38 44 47 54
17 16 16 15 11
50 51 48 48 46
54 52 53 51 56
21 25 27 30 32 30 36
79 77 79 80 80 84
18 19 18 17 14 13 13
8 15 16 18 20 20 26
42 42 44 48 49 49
44 37 34 31 30 26 25 25
76 80 81 82 88 92
27 27 22 21 18 15 15
26 26 25 19 17 13
35 32 34 32 31 25
37 41 43 50 50
19 18 16 11 8 6 8
18 21 19 13 6
57 58 56 53 51 47 46
58 57 54 51 52 52
73 72 75 81 84 88
14 13 18 20 21 24
65 65 62 61 59 59 59
76 80 80 83 85 88 91 97
89 85 81 80 77
23 23 23 26 27 25
71 69 70 69 66
29 31 36 37 40 43 45 45
77 74 71 71 71
44 47 44 41 44 39
69 74 77 79 78 75
55 55 57 60 63 62 66
74 73 71 69 69 66 59
25 21 20 17 16 13 13 14
26 30 31 29 31 33 33
37 41 44 45 48 52 53
67 67 68 65 72
76 76 79 77 79 80 83 82
28 30 33 35 33 31
48 54 56 57 61 62 59
31 26 25 24 22 20
10 11 13 13 20
3 3 7 10 12 14 18
75 79 76 79 80 81
16 21 27 29 31 33 35 40
24 20 19 18 18 17 10
90 93 90 83 82 80 79
37 35 33 30 29 27 26 24
20 17 15 13 11 10 9
1 2 5 6 9 11 13 15
42 44 46 48 50 52 54 57
40 38 36 34 31 28
24 23 20 17 16 15 13 12
78 77 75 72 71 68 67
65 66 69 71 74 77 79 82
65 64 62 59 58
70 67 66 64 62
23 21 20 19 16
26 29 30 33 35 36
50 48 47 45 44 41 38
9 12 13 15 17 18 20 21
43 41 40 38 37 34 32
24 23 21 19 16 13 10 9
93 91 88 86 83 80
37 34 33 30 29
36 33 30 29 28 26 23 21
84 87 90 91 94 95 96
57 55 52 51 50 49 47
83 86 87 88 89 92 94 96
56 58 59 62 63 64 67 70
62 65 66 68 69 72 74
52 54 57 59 60 63
46 43 40 39 37
70 69 68 65 63 60
72 73 76 78 79 81
23 24 25 26 29
69 66 65 62 59
49 50 53 54 56 58
39 40 43 44 46 49
50 48 47 45 42 41
21 18 15 14 13 11 8 7
66 68 69 70 72 75
64 61 59 57 55 54 53
22 25 26 29 30 33 36
35 38 41 43 46
22 21 19 17 14 12 10 7
35 33 30 28 25 24
81 79 76 75 73
48 45 42 41 38 37
24 25 27 29 31 32
18 21 22 24 26 28 31 32
21 24 25 26 29 30 32
87 85 83 81 80 77 74 71
35 32 29 27 24 22 19 16
89 91 92 93 96 97
37 35 34 33 32 31 29
84 81 80 77 74 71 69
38 35 32 29 26 25
93 90 89 87 84 82 80 78
88 85 83 81 79 78 76 73
2 3 5 6 9 11 14 17
31 30 27 25 23 20
95 92 91 89 87 85 82
6 7 9 12 13 15 17 20
48 45 42 41 40 39 36 35
76 73 72 69 66 64
82 80 77 75 73 72
67 69 71 72 75 76 78 81
1 2 4 6 9 12 13
21 20 19 16 13 11 9 8
42 39 38 37 34 32
21 22 25 27 30 33 36
57 56 55 54 53 51 48 47
37 35 33 30 27 25 23
14 17 18 21 23 26 27 29
43 45 48 50 51 52 53 55
15 14 13 10 8
17 18 21 22 25 26
13 10 9 8 6 5 4
84 86 88 91 93
91 90 87 84 83
35 32 30 29 28 26
56 59 61 62 65
20 22 24 27 28
32 33 35 36 39
48 49 50 51 53 54 56 58
60 62 64 67 69 71
25 22 19 17 14 12 11
25 28 30 31 33 35 37 38
56 54 51 50 47 46 43
31 32 33 35 38
59 58 56 54 52 50 47 46
86 89 90 93 94 95 96 99
95 93 92 91 88 85
65 64 63 60 58 57 54
74 76 77 80 82
76 78 79 81 82
12 11 10 7 6 4 2
90 87 86 83 82
16 13 12 11 9 8 7
41 38 36 35 32 31 28 26
43 45 46 48 51 52 54 55
94 91 88 85 82 81
77 74 72 69 68 66 65 63
30 32 35 36 37 40 42
82 83 85 87 88 90
67 64 62 60 59 57 54
92 90 89 88 87 84 83 82
21 22 25 27 30
63 65 68 69 71
6 9 11 12 13 16
92 89 88 86 83 80
92 91 89 87 86 85 84 82
18 16 13 11 9
52 50 47 44 41 39 38
10 11 13 15 18
46 44 41 39 38 37 34
27 30 31 33 36
65 67 69 72 73 75 76 77
40 43 46 47 50
88 85 82 79 77 76 74
87 89 91 94 95
88 86 83 80 79 76 73 70
41 43 45 48 51 52 55
87 90 91 93 95
79 82 85 88 89 91 92
7 10 11 14 16 17 19
46 47 48 50 51 52
95 93 90 88 86 84 83 82
68 70 71 73 76
80 79 76 73 71 69
80 83 84 87 89 90
94 93 92 91 88 87
85 82 80 77 74 71 68
32 29 28 25 24 22 20
45 42 40 37 35 33
51 48 47 44 41 39
33 36 38 41 42 45
84 82 81 78 76 75 72 69
24 25 28 29 30 32 33 36
75 77 79 81 82 85 88
2 5 6 9 10
73 75 77 80 82 84 86
69 68 65 62 59
82 81 78 77 75 74
69 71 72 73 76 77 79 80
78 80 82 85 87
95 94 91 90 88 85 84 81
65 68 70 71 72 75 77
80 81 83 85 86 87
42 39 36 34 31 29
82 85 87 88 90 91 92 93
44 41 40 37 36 35 32
71 69 67 65 62 60 58 56
92 89 87 84 81 80 77
31 30 28 26 23 22 19 17
14 16 19 22 23 26
84 86 88 90 91
37 34 33 32 29
75 76 79 82 85 86 87 90
63 66 69 70 73 75
26 27 28 31 33 34 37 39
46 43 40 37 34 32
28 26 25 23 21 19
67 65 63 60 57 55 52
33 35 36 39 42 43 45 46
58 55 52 49 46 45
99 98 97 95 92 90 88
27 30 32 35 36 39 41 43
59 58 57 56 54 53 50 47
31 29 27 25 24 22 21 18
35 33 32 29 28 25 23 20
66 65 62 60 58 56 53 50
50 51 54 55 58 61
71 68 65 63 62
47 48 51 53 55 58 59 61
10 12 14 16 19 21
33 35 37 38 39 41 44 45
94 92 91 90 89
10 12 15 17 19 22
83 86 87 89 91 93 95 98
27 24 22 21 18 15 13
97 95 92 89 88 87
20 21 24 25 28 29 31
72 71 70 69 67 65 64
65 62 59 56 55 52 51 48
79 77 75 72 71 69
70 67 65 63 61 59 56
9 10 12 14 16 17 18 21
9 10 13 15 17 19
49 50 53 56 57
51 54 56 57 58 61 63 66
86 88 90 92 93
37 38 40 42 43
32 33 35 37 38 39
77 75 72 69 68 65 63 60
10 13 15 17 19
29 31 34 35 36 38
61 63 64 67 69 71 74
86 84 82 81 79 76 74
51 48 47 45 44 42 39 37
37 39 42 45 46 49 50
54 57 59 60 62 63 65 66
43 45 48 49 51 52
57 56 55 54 52 50 47
13 15 18 19 20 23 26
4 7 9 12 13 14
24 23 20 19 16 15
53 52 50 49 47 44 43
11 13 15 17 20 22 24
27 26 24 23 22 20
4 5 6 9 11
26 23 21 19 17 14 13 10
34 35 37 39 41 44 46
59 61 63 65 68 71
50 47 46 44 42 41 39 36
23 20 17 15 13
28 25 23 20 17 14 12
2 5 7 10 13 16
16 18 19 20 23 25
5 6 7 8 11 14
44 47 48 50 53 54
43 42 40 39 36 35 34 31
57 59 61 62 65 66 67
37 36 35 33 30 29 28 26
80 79 78 76 73 70
93 92 90 87 86 84
54 52 49 47 46 44 43 41
27 25 22 19 18 15
76 78 80 81 83 85
79 77 74 71 69 67 66
29 28 25 24 21 18 16
2 3 6 9 10
12 9 6 3 2
55 58 60 61 62 63 64 66
16 17 18 20 21 22 25 28
62 63 64 66 68 71 74
17 20 21 23 26 27 29
22 19 18 17 16
44 47 48 51 52 54 56
36 38 41 42 44 46 48 50
28 27 24 23 20 19 17
20 17 15 13 11
65 64 61 60 58 55
49 46 44 43 42 39 37 34
75 73 72 71 68
50 51 53 54 56 57
73 72 69 68 66 63 61
44 45 48 51 52 53
66 65 63 60 59 57
48 49 50 51 54 57 58 59
71 74 77 79 82 84 86 87
60 58 55 53 51 49
12 14 15 16 19
35 36 37 38 40 43 46
20 19 18 17 15 13 12 9
92 89 88 85 83 81
14 15 17 20 21 22 24 27
74 76 78 79 80 82 83 86
61 58 55 52 50 49 46
48 49 50 51 54 57 58 60
86 87 88 90 93 95
49 47 44 41 39 37
10 9 8 7 5
78 80 81 82 83 85 88 91
51 50 48 45 44 41 40
52 50 48 46 44
88 89 90 91 94 95 96
91 89 87 86 84
26 24 23 21 20 18 17
61 60 59 58 57 54 51 50
20 23 25 28 30
39 42 45 46 48 51
49 51 53 55 58 61
20 22 23 24 25 28 30 33
50 52 54 56 59
83 82 81 80 78
66 63 62 60 57
60 58 57 54 51 48 46 45
86 87 90 92 93 95 97 98
14 16 17 19 21
42 41 40 38 35
63 66 68 69 71
78 79 81 84 85 86 87 89
18 17 16 13 11 8
66 68 71 74 77 79
79 81 84 87 88 91
45 46 47 48 50 51
25 26 29 30 31 32 35 36
59 60 63 64 66
89 86 85 83 80 77 75
31 29 27 26 23 21
29 31 34 37 38
78 79 80 82 84 86 89 90
66 64 63 61 58
53 56 58 59 62 64 67 68
14 11 8 7 4
57 59 61 63 65
30 29 27 25 24
29 27 26 23 21 19 16
38 37 36 33 30 28
19 21 24 27 29 32 33 35
69 68 65 64 63 61 59
84 83 82 81 80 79
10 12 14 16 18 21 24
40 41 42 44 47 50 52
30 29 28 27 24 23
62 60 59 57 54
53 52 50 49 48 45 44 41
28 26 25 22 20
64 61 58 55 54 53 51
63 65 66 69 70
52 50 49 47 44 43 40 39
25 27 28 30 31 33 36 37
87 89 92 93 94 96 99
94 91 88 87 85 82 79 76
66 64 63 61 58 57
60 58 57 55 52 49 48
2 3 6 8 11 12 15
34 33 31 30 29 26 24
70 71 73 74 76 78 79
19 22 23 26 29 31 32
68 69 70 72 74 77
73 71 69 68 65
23 24 27 29 32 33
57 56 55 52 51 50 48 46
20 22 24 26 27 29
78 81 84 87 89 91 94
74 75 78 79 81 84 87
48 47 45 44 41 38 35
33 34 37 40 43 45
74 76 79 80 82 84
15 17 20 21 22 25 28 29
7 10 12 14 15 16 19 20
51 48 45 42 39 38 37
1 4 7 9 12 14
82 81 80 78 75 74 73 72
39 37 36 34 32 29
9 11 14 16 18 20
8 9 10 11 13 15 17 20
20 22 23 25 28 29 30
87 84 83 80 79 77 74 72
50 52 55 57 60 62 64 67
75 72 71 69 66 65 62
35 33 31 29 27 24 22
78 79 82 84 86 87 90
16 15 14 11 9 8 6 5
56 58 59 61 64 66 69
37 35 33 31 30
14 16 18 19 22
14 15 18 21 22 25
83 84 86 87 90 92 95
78 80 81 83 85 86
3 4 5 8 9
68 70 73 75 76 77
65 63 62 59 56
18 19 20 21 24 27 29 31
45 47 49 50 53 55 58
45 43 41 38 35 32 30
53 54 55 57 58
90 88 86 85 84 81 78
6 8 10 11 14 15
18 21 23 26 29 31 34
51 50 47 46 43 41 39 37
82 81 78 75 72 71 68
98 95 94 93 90 88 85
20 19 16 14 12
11 14 16 18 19 22 24 27
29 27 24 23 22
19 18 16 13 10 7 5 4
44 41 38 36 34
45 44 42 40 39 37
38 36 35 32 30 29 26
27 28 29 30 33 36
68 69 70 72 73 74
89 90 93 94 96
44 42 41 39 38 35
85 82 81 80 77
32 34 35 36 37 39 41
44 43 42 40 37 36
84 82 80 79 78 75 74 72
40 42 45 48 49 51 52
77 78 80 81 82 84 87 90
37 40 42 43 46
92 90 88 87 84
37 39 40 41 44 45 46
72 75 78 81 83 86 88 91
66 67 69 70 73
40 41 42 44 45
92 90 87 86 83 82 79
33 34 37 38 41
75 77 78 81 83 84 87 90
66 65 63 60 59
69 68 66 65 62
68 71 73 75 76
79 82 85 87 90
61 58 56 55 54 52
6 7 9 12 13 14
25 24 22 20 18 17 14 13
48 51 52 53 56 59
50 48 47 46 43 41 39
3 5 6 9 10
44 42 40 39 38
47 46 43 40 39
24 26 29 31 34 37 40 41
13 15 18 21 22 23 25 28
52 51 50 49 47 46 45 43
48 47 44 42 39 36 34
60 58 56 53 50 49
26 23 20 17 16 15
77 74 72 71 68 67 65 62
9 10 12 13 16 17 19 21
51 54 56 57 58 59 60
54 57 59 61 64 65
35 38 40 42 44
37 38 40 43 45 48
37 38 40 43 45 47 48
80 81 83 84 87
35 37 40 41 43
80 78 75 74 71 68 66
46 48 49 52 53 55 56
53 54 57 58 59 61 64 67
51 54 56 59 62 65 66 67
21 24 26 27 28 30
27 26 24 22 20 19 16
19 21 23 25 28 29 31 32
10 9 8 7 5 4
32 35 36 39 40 42 44 46
51 49 48 46 45 42
51 48 46 45 43
52 55 56 59 61 63 66
19 17 15 12 10 9 6 5
5 6 7 10 13
46 44 43 40 38 36 33
24 22 21 18 15 14 12
50 53 54 57 59
78 80 82 83 85
14 15 16 17 18 21
37 34 33 32 31 28 26
55 57 59 62 65 66 69
87 90 93 95 96 99
37 39 42 44 45
1 4 5 6 8
24 21 20 19 16 15 13
75 73 72 71 69 67
77 79 82 85 87 90
45 42 39 38 35 33 31
34 31 30 27 25 23
40 39 36 33 30
77 80 82 85 87 90 91
76 74 72 70 67
58 60 63 66 69
43 41 40 39 38
46 44 42 40 39
96 94 93 91 88
90 89 88 86 83 80 77
68 67 66 64 62 60 58
87 90 93 94 95 96 99
30 32 33 35 37
41 44 45 46 47
44 43 41 40 37
16 14 11 9 6
97 95 93 91 89 87
23 21 19 16 15
63 60 57 55 53 51 50 47
60 62 65 67 70 73
6 8 10 11 12 15 16
79 78 77 74 71 70
56 57 59 62 63 66
46 43 42 40 38 37 35 32
87 85 83 81 79 77 76 75
74 72 70 69 66
17 14 13 12 9
74 76 77 78 79 81
18 21 23 25 27 29 32
34 31 29 27 25 23 20
86 84 83 82 80 78 75
41 42 44 45 48 51 54 57
36 34 32 31 30 27 24
42 39 37 36 35 32 30 28
84 82 79 78 76 74
42 40 37 36 35 34 31
57 56 54 52 50 48 46
20 21 24 26 27
37 40 41 44 46 48 50 51
28 29 30 31 34
12 14 16 18 21
69 70 73 75 76 77
40 39 36 34 32 30 29 26
57 59 61 64 66 69 70
35 36 37 39 42
56 55 54 53 50 49 48
60 59 58 57 55
33 30 29 27 26 24 22
85 82 81 80 78 76 75
67 64 61 60 57 54 52
86 85 84 81 78
47 45 43 42 41
25 24 23 20 19 18 17 15
58 57 54 53 50 49
88 90 91 92 93 96 99
46 43 41 38 35
20 18 15 13 12 9 7
81 79 77 74 73 72 69 67
61 58 57 54 52 49 48
93 91 90 87 85 84 81 79
9 10 12 13 15 18 19 21
66 69 72 75 77 79 82 83
73 74 76 79 80 81 84
61 59 57 56 55
18 15 12 10 9 8 5
85 82 79 76 74
40 38 36 34 33
69 72 74 77 80 82 83 85
70 71 73 75 77 80 82
78 77 75 73 71 70 69 68
3 6 8 9 10 11 14
5 6 9 12 14
60 58 55 54 52 49 48
8 11 14 15 16
65 66 69 70 72 73 74 77
79 82 84 87 88
61 59 58 55 52 51
44 41 40 39 37 34 32
23 25 28 29 31 32 35
33 36 37 39 41 42 44 46
51 48 45 42 41
62 59 58 56 55 52 49
28 30 32 33 34 35 37 40
52 54 55 57 58 59 62
20 19 16 14 13 10 8 5
60 61 64 67 69
71 69 67 66 63
98 96 95 92 90
6 9 12 13 15 18 19 21
35 36 38 41 43 45 48 50
91 88 85 84 81 78 76
83 85 87 90 91 94 97 98
64 66 69 70 71 72 75 76
57 55 52 51 49
15 17 20 23 25`,I=c.array(c.string().transform(r=>r.split(" ")).pipe(c.array(c.coerce.number()))),v=I.parse(x.split(`
`));function V(r){let s;for(const a of r){if(s!==void 0&&s>a)return!1;s=a}return!0}function z(r){let s;for(const a of r){if(s!==void 0&&s<a)return!1;s=a}return!0}function R(r,s,a){let t;for(const i of r){if(t!==void 0){const n=Math.abs(t-i);if(n<s||n>a)return!1}t=i}return!0}function B(){const[r,s]=o.useState("unstarted"),[a,t]=o.useState(0),[i,n]=o.useState(0),j=o.useCallback(async()=>{s("in-progress"),n(0),t(0);for await(const d of v)t(l=>l+1),(V(d)||z(d))&&R(d,1,3)&&n(l=>l+1),await b(1);s("complete")},[s,n,t]);return e.jsxs(e.Fragment,{children:[e.jsxs("div",{className:"mt-4 flex gap-x-4",children:[e.jsx("div",{children:e.jsx(y,{input:x})}),e.jsx("div",{children:e.jsx(w,{size:"lg",onClick:j,disabled:r==="in-progress",children:"Process Input"})})]}),e.jsxs("div",{className:"mt-4 tabular-nums",children:[e.jsxs("div",{children:["Progress: ",a," / ",v.length]}),e.jsxs("div",{children:["Total: ",i]})]})]})}const C=c.array(c.string().transform(r=>r.split(" ")).pipe(c.array(c.coerce.number()))),f=C.parse(x.split(`
`));function H(r){let s;for(const a of r){if(s!==void 0&&s>a)return!1;s=a}return!0}function N(r){let s;for(const a of r){if(s!==void 0&&s<a)return!1;s=a}return!0}function U(r,s,a){let t;for(const i of r){if(t!==void 0){const n=Math.abs(t-i);if(n<s||n>a)return!1}t=i}return!0}function A(){const[r,s]=o.useState("unstarted"),[a,t]=o.useState(0),[i,n]=o.useState(0),j=o.useCallback(async()=>{s("in-progress"),n(0),t(0);for await(const d of f){t(l=>l+1);for(let l=-1;l<f.length;l++){const p=d.filter((m,P)=>P!==l);(H(p)||N(p))&&U(p,1,3)&&(n(m=>m+1),l=f.length)}await b(1)}s("complete")},[s,n,t]);return e.jsxs(e.Fragment,{children:[e.jsxs("div",{className:"mt-4 flex gap-x-4",children:[e.jsx("div",{children:e.jsx(y,{input:x})}),e.jsx("div",{children:e.jsx(w,{size:"lg",onClick:j,disabled:r==="in-progress",children:"Process Input"})})]}),e.jsxs("div",{className:"mt-4 tabular-nums",children:[e.jsxs("div",{children:["Progress: ",a," / ",f.length]}),e.jsxs("div",{children:["Total: ",i]})]})]})}const K=function(){return e.jsxs(e.Fragment,{children:[e.jsx(T,{to:k.fullPath}),e.jsx(h,{hash:"part-1-prompt",children:"Part 1 – Prompt"}),e.jsx("p",{children:"Fortunately, the first location The Historians want to search isn't a long walk from the Chief Historian's office."}),e.jsxs("p",{children:["While the"," ",e.jsx("a",{href:"https://adventofcode.com/2015/day/19",target:"_blank",children:"Red-Nosed Reindeer nuclear fusion/fission plant"})," ","appears to contain no sign of the Chief Historian, the engineers there run up to you as soon as they see you. Apparently, they ",e.jsx("em",{children:"still"})," talk about the time Rudolph was saved through molecular synthesis from a single electron."]}),e.jsx("p",{children:"They're quick to add that - since you're already here - they'd really appreciate your help analyzing some unusual data from the Red-Nosed reactor. You turn to check if The Historians are waiting for you, but they seem to have already divided into groups that are currently searching every corner of the facility. You offer to help with the unusual data."}),e.jsxs("p",{children:["The unusual data (your puzzle input) consists of many ",e.jsx("em",{children:"reports"}),", one report per line. Each report is a list of numbers called ",e.jsx("em",{children:"levels"})," that are separated by spaces. For example:"]}),e.jsx(S,{content:`7 6 4 2 1
1 2 7 8 9
9 7 6 2 1
1 3 2 4 5
8 6 4 4 1
1 3 6 7 9`}),e.jsx("p",{children:"This example data contains six reports each containing five levels."}),e.jsxs("p",{children:["The engineers are trying to figure out which reports are ",e.jsx("em",{children:"safe"}),". The Red-Nosed reactor safety systems can only tolerate levels that are either gradually increasing or gradually decreasing. So, a report only counts as safe if both of the following are true:"]}),e.jsxs("ul",{children:[e.jsxs("li",{children:["The levels are either ",e.jsx("em",{children:"all increasing"})," or ",e.jsx("em",{children:"all decreasing"}),"."]}),e.jsxs("li",{children:["Any two adjacent levels differ by ",e.jsx("em",{children:"at least one"})," and"," ",e.jsx("em",{children:"at most three"}),"."]})]}),e.jsx("p",{children:"In the example above, the reports can be found safe or unsafe by checking those rules:"}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsx("code",{children:"7 6 4 2 1"}),": ",e.jsx("em",{children:"Safe"})," because the levels are all decreasing by 1 or 2."]}),e.jsxs("li",{children:[e.jsx("code",{children:"1 2 7 8 9"}),": ",e.jsx("em",{children:"Unsafe"})," because ",e.jsx("code",{children:"2 7"})," is an increase of 5."]}),e.jsxs("li",{children:[e.jsx("code",{children:"9 7 6 2 1"}),": ",e.jsx("em",{children:"Unsafe"})," because ",e.jsx("code",{children:"6 2"})," is a decrease of 4."]}),e.jsxs("li",{children:[e.jsx("code",{children:"1 3 2 4 5"}),": ",e.jsx("em",{children:"Unsafe"})," because ",e.jsx("code",{children:"1 3"})," is increasing but ",e.jsx("code",{children:"3 2"})," is decreasing."]}),e.jsxs("li",{children:[e.jsx("code",{children:"8 6 4 4 1"}),": ",e.jsx("em",{children:"Unsafe"})," because ",e.jsx("code",{children:"4 4"})," is neither an increase or a decrease."]}),e.jsxs("li",{children:[e.jsx("code",{children:"1 3 6 7 9"}),": ",e.jsx("em",{children:"Safe"})," because the levels are all increasing by 1, 2, or 3."]})]}),e.jsxs("p",{children:["So, in this example,"," ",e.jsx("code",{children:e.jsx("em",{children:"2"})})," ","reports are ",e.jsx("em",{children:"safe"}),"."]}),e.jsxs("p",{children:["Analyze the unusual data from the engineers. ",e.jsx("em",{children:"How many reports are safe?"})]}),e.jsx(h,{hash:"part-1-solution",children:"Part 1 – Solution"}),e.jsx(B,{}),e.jsxs("p",{children:[e.jsx("code",{children:"zod"})," did the heavy lifting for initial data parsing, of course:"]}),e.jsx(u,{content:`import { z } from "zod";
import { input } from "./input";

const parser = z.array(
    z
        .string()
        .transform((value) => value.split(" "))
        .pipe(z.array(z.coerce.number()))
);
const values = parser.parse(input.split("\\n"));`}),e.jsx("p",{children:"I spent a few minutes making sure these functions were correct:"}),e.jsx(u,{content:`function allIncreaseing(values: Array<number>) {
    let lastValue;

    for (const value of values) {
        if (lastValue !== undefined && lastValue > value) {
            return false;
        }

        lastValue = value;
    }

    return true;
}

function allDecreasing(values: Array<number>) {
    let lastValue;

    for (const value of values) {
        if (lastValue !== undefined && lastValue < value) {
            return false;
        }

        lastValue = value;
    }

    return true;
}

function allDifferBy(values: Array<number>, min: number, max: number) {
    let lastValue;

    for (const value of values) {
        if (lastValue !== undefined) {
            const diff = Math.abs(lastValue - value);

            if (diff < min || diff > max) {
                return false;
            }
        }

        lastValue = value;
    }

    return true;
}`}),e.jsx("p",{children:"Then we just iterate over all of the rows and check against these functions."}),e.jsx(u,{content:"(allIncreaseing(value) || allDecreasing(value)) && allDifferBy(value, 1, 3)"}),e.jsx(g,{}),e.jsx(h,{hash:"part-2-prompt",children:"Part 2 – Prompt"}),e.jsxs("p",{children:["The engineers are surprised by the low number of safe reports until they realize they forgot to tell you about the"," ",e.jsx("span",{title:"I need to get one of these!",children:"Problem Dampener"}),"."]}),e.jsxs("p",{children:["The Problem Dampener is a reactor-mounted module that lets the reactor safety systems ",e.jsx("em",{children:"tolerate a single bad level"})," in what would otherwise be a safe report. It's like the bad level never happened!"]}),e.jsx("p",{children:"Now, the same rules apply as before, except if removing a single level from an unsafe report would make it safe, the report instead counts as safe."}),e.jsx("p",{children:"More of the above example's reports are now safe:"}),e.jsxs("ul",{children:[e.jsxs("li",{children:[e.jsx("code",{children:"7 6 4 2 1"}),": ",e.jsx("em",{children:"Safe"})," without removing any level."]}),e.jsxs("li",{children:[e.jsx("code",{children:"1 2 7 8 9"}),": ",e.jsx("em",{children:"Unsafe"})," regardless of which level is removed."]}),e.jsxs("li",{children:[e.jsx("code",{children:"9 7 6 2 1"}),": ",e.jsx("em",{children:"Unsafe"})," regardless of which level is removed."]}),e.jsxs("li",{children:[e.jsxs("code",{children:["1 ",e.jsx("em",{children:"3"})," 2 4 5"]}),": ",e.jsx("em",{children:"Safe"})," by removing the second level, ",e.jsx("code",{children:"3"}),"."]}),e.jsxs("li",{children:[e.jsxs("code",{children:["8 6 ",e.jsx("em",{children:"4"})," 4 1"]}),": ",e.jsx("em",{children:"Safe"})," by removing the third level, ",e.jsx("code",{children:"4"}),"."]}),e.jsxs("li",{children:[e.jsx("code",{children:"1 3 6 7 9"}),": ",e.jsx("em",{children:"Safe"})," without removing any level."]})]}),e.jsxs("p",{children:["Thanks to the Problem Dampener,"," ",e.jsx("code",{children:e.jsx("em",{children:"4"})})," ","reports are actually ",e.jsx("em",{children:"safe"}),"!"]}),e.jsxs("p",{children:["Update your analysis by handling situations where the Problem Dampener can remove a single level from unsafe reports. ",e.jsx("em",{children:"How many reports are now safe?"})]}),e.jsx(h,{hash:"part-2-solution",children:"Part 2 – Solution"}),e.jsx(A,{}),e.jsxs("p",{children:["I used the same ",e.jsx("code",{children:"zod"})," parser and functions from Part 1 (",e.jsx("code",{children:"allIncreaseing"}),", ",e.jsx("code",{children:"allDecreasing"}),", ",e.jsx("code",{children:"allDifferBy"}),")."]}),e.jsx("p",{children:"The only real change is that we need to check sub-arrays with a single value removed (as well as the original array) for each row."}),e.jsx(u,{content:`// Start with i = -1 so that the filter doesn't remove any values for the first iteration
// This lets us check the whole row first, then check sub-rows after
for (let i = -1; i < row.length; i++) {
    const subRow = row.filter((_, index) => index !== i);

    if ((allIncreaseing(subRow) || allDecreasing(subRow)) && allDifferBy(subRow, 1, 3)) {
        runningTotal += 1;

        // Early exit if we find a match
        i = values.length;
    }
}`}),e.jsx(g,{}),e.jsx(h,{hash:"recap",children:"Recap"}),e.jsxs(D,{children:[e.jsx("thead",{children:e.jsxs("tr",{children:[e.jsx("th",{children:"Part 1 Time"}),e.jsx("th",{children:"Part 1 Rank"}),e.jsx("th",{children:"Part 2 Time"}),e.jsx("th",{children:"Part 2 Rank"})]})}),e.jsx("tbody",{children:e.jsxs("tr",{children:[e.jsx("td",{children:"00:19:10"}),e.jsx("td",{children:"6,592"}),e.jsx("td",{children:"00:23:18"}),e.jsx("td",{children:"3,459"})]})})]}),e.jsxs("p",{children:["This was fairly easy, I just spent a lot of time on the Part 1 making sure my helper functions were correct. The ",e.jsx("code",{children:"allIncreasing"})," and"," ",e.jsx("code",{children:"allDecreasing"})," functions only took a minute or so each, but"," ",e.jsx("code",{children:"allDifferBy"})," took a bit longer ¯\\_(ツ)_/¯."]})]})};export{K as component};
